...
public static partial class Module
{
    ...
    // simple table to store metadata about game state
    ...
    {
        Player player = ctx.Db.player.identity.Find(ctx.Sender) ?? throw new Exception("Player not found");
        // remove any circles from the arena
        foreach (var circle in ctx.Db.circle.player_id.Filter(player.player_id))
        {
            Entity entity = ctx.Db.entity.entity_id.Find(circle.entity_id) ?? throw new Exception("Could not find circle");
    ...
    const uint TARGET_FOOD_COUNT = 200;
    ...
    public static void SpawnFood(ReducerContext ctx, SpawnFoodTimer _timer)
    {
        ...
        {
            var mass = rng.Range(FOOD_MASS_MIN, FOOD_MASS_MAX);
            var radius = MassToRadius(mass);

            DbVector2 position = new DbVector2(
                rng.Range(radius, world_size - radius),
                rng.Range(radius, world_size - radius)
            );

            Entity entity = ctx.Db.entity.Insert(
                new Entity { position = position, mass = mass }
            );
            ...
            food_count++;
            Log.Info($"Spawned food at {position.x}, {position.y}");
        }
    }
    ...
    public static Entity SpawnPlayerInitialCircle(ReducerContext ctx, uint player_id)
    {
        var rng = ctx.Rng;
        var world_size = (ctx.Db.config.id.Find(0) ?? throw new Exception("Config not found")).world_size;
        var radius = MassToRadius(START_PLAYER_MASS);
        ...
        return SpawnCircleAt(ctx, player_id, START_PLAYER_MASS, position, ctx.Timestamp);
    }
        ...
        foreach (var circle in ctx.Db.circle.Iter())
        {
            ...
            foreach (var entity in ctx.Db.entity.Iter())
            {
                ...
                if (IsOverlapping(circle_entity, entity))
                {
                    ...
                    if (other_circle.HasValue && other_circle.Value.player_id != circle.player_id)
                    {
