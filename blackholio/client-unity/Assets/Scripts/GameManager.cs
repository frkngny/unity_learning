...
public class GameManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        // In order to build a connection to SpacetimeDB, we need to register our callbacks
        // and specify a SpacetimeDB server URI and module name
        ...
        // if the user has a SpacetimeDB auth token stored in the Unity PlayerPrefs, use it
    ...
    {
        CreateBorderCube(
            new Vector2(worldSize / 2.0f, worldSize + borderThickness / 2),
            new Vector2(worldSize + borderThickness * 2.0f, borderThickness)
        ); // North
        CreateBorderCube(
            new Vector2(worldSize / 2.0f, -borderThickness / 2),
            new Vector2(worldSize + borderThickness * 2.0f, borderThickness)
        ); // South
        CreateBorderCube(
            new Vector2(worldSize + borderThickness / 2, worldSize / 2.0f),
            new Vector2(borderThickness, worldSize + borderThickness * 2.0f)
        ); // East
        CreateBorderCube(
            new Vector2(-borderThickness / 2, worldSize / 2.0f),
            new Vector2(borderThickness, worldSize + borderThickness * 2.0f)
        ); // West
